...
/** Static class for packing update data. When a specific property of a part
  * changes and needs sending to the client, a bit can be set in the mask. This
  * bit can then be checked in the writeScheduled callback. This prevents
  * sending multiple packets if the same property updates more than once per
  * tick.
  */
  ...
  def schedulePacket(part: TMultiPart, mask: Long) {
    if (part.world.isRemote)
      throw new IllegalArgumentException(
        "Cannot use PacketScheduler on a client world"
      )
...
trait IScheduledPacketPart {
  ...
  /** Write scheduled data to the packet, mask is the cumulative mask from calls
    * to schedulePacket
    */
  def writeScheduled(mask: Long, packet: MCDataOutput)

  /** Returns the width (in bytes) of the data type required to hold all valid
    * mask bits. Valid values are 1, 2, 4 and 8
    */
  def maskWidth: Int

  /** Read data matching mask. Estiablishes a method for subclasses to override.
    * This should be called from read
    */
