...
class StackAnalyser(val owner: Type, val m: MethodNode) {
    ...
    if (e.getType.getSize == 2) {
      if (peek(i) != e)
        throw new IllegalStateException(
          "Wide stack entry elems don't match (" + e + "," + peek(i)
        )
      _pop(i)
    }
    e
  ...
  def visitInsn(ainsn: AbstractInsnNode) {
    ...
    ainsn match {
      ...
      case incinsn: IincInsnNode =>
        ainsn.getOpcode match {
          case IINC =>
            setL(
              incinsn.`var`,
              Store(
                BinaryOp(IINC, Const(incinsn.incr), Load(locals(incinsn.`var`)))
              )
            )
        }
      case jinsn: JumpInsnNode =>
        ainsn.getOpcode match {
      ...
      case lnode: LabelNode =>
        catchHandlers.get(lnode) match {
          case Some(tcblock) =>
            push(CaughtException(Type.getType(tcblock.`type`)))
          case None =>
        }
      case _ =>
    }
