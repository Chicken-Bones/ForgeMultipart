...
/** Mixin trait implementation for IRedstonePart Provides and overrides various
  * redstone functions.
  */
trait TRedstoneTile extends TileMultipart with IRedstoneTile {
  ...
  override def weakPowerLevel(side: Int): Int =
    weakPowerLevel(
      side,
      otherConnectionMask(getWorldObj, xCoord, yCoord, zCoord, side, true)
    )

  override def canConnectRedstone(side: Int): Boolean = {
    val vside = vanillaToSide(side)
    return (getConnectionMask(vside) & otherConnectionMask(
      getWorldObj,
      xCoord,
      yCoord,
      zCoord,
