...
/** Mixin trait implementation for parts implementing IFluidHandler. Distributes
  * fluid manipulation among fluid handling parts.
  */
trait TFluidHandlerTile extends TileMultipart with IFluidHandler {
  ...
  override def canFill(dir: ForgeDirection, liquid: Fluid) =
    tankList.find(_.canFill(dir, liquid)).isDefined

  override def canDrain(dir: ForgeDirection, liquid: Fluid) =
    tankList.find(_.canDrain(dir, liquid)).isDefined
  ...
  override def drain(
    ...
    tankList.foreach { p =>
      ...
      if (
        ret != null && ret.amount > 0 && (drained == null || drained
          .isFluidEqual(ret))
      ) {
        if (doDrain)
