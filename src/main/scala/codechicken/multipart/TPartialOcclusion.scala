...
/** This class provides a special type of occlusion model used by microblocks.
  * The partial occlusion test defines bounding boxes that may intersect, so
  * long as no part is completely obscured by a combination of the others.
  * Partial bounding boxes may not intersect with normal bounding boxes from
  * NormalOcclusionTest
  *
  * This test is actually managed by the mixin trait TPartialOcclusionTile which
  * is generated when the marker interface JPartialOcclusion is found
  */
class PartialOcclusionTest(size: Int) {
  ...
  def fill(v: Int, box: Cuboid6) {
    for (x <- (box.min.x * res + 0.5).toInt until (box.max.x * res + 0.5).toInt)
      for (
        y <- (box.min.y * res + 0.5).toInt until (box.max.y * res + 0.5).toInt
      )
        for (
          z <- (box.min.z * res + 0.5).toInt until (box.max.z * res + 0.5).toInt
        ) {
          val i = (x * res + y) * res + z
          if (bits(i) == 0)
            bits(i) = v.toByte
