...
/** This suite of 3 classes provides simple functions for standard bounding box
  * based occlusion testing. If any two parts have overlapping bounding boxes,
  * the test fails
  *
...
object NormalOcclusionTest {
  ...
  /** Performs the test, returns true if the test fails
    */
  def apply(
      boxes1: Traversable[Cuboid6],
      boxes2: Traversable[Cuboid6]
  ): Boolean =
  ...
  def apply(part1: JNormalOcclusion, part2: TMultiPart): Boolean = {
    ...
    if (part2.isInstanceOf[JPartialOcclusion])
      boxes =
        boxes ++ part2.asInstanceOf[JPartialOcclusion].getPartialOcclusionBoxes

    return NormalOcclusionTest(boxes, part1.getOcclusionBoxes)
  }
}

/** Java interface containing callbacks for normal occlusion testing. Make sure
  * to override occlusionTest as in TNormalOcclusion
  */
...
/** Utility part class for performing 3rd party occlusion tests
  */
class NormallyOccludedPart(bounds: Iterable[Cuboid6])
    extends TMultiPart
