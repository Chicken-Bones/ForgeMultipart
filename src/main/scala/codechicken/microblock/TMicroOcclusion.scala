...
/** Suite of classes for performing microblock style occlusion and shrink
  * rendering. Use at your own peril.
  */
object MicroOcclusion {
  ...
  def shrinkFrom(
      p: JMicroShrinkRender,
      other: JMicroShrinkRender,
    ...
    if (shrinkTest(p, other)) {
      shrink(
        renderBounds,
        other.getBounds,
        shrinkSide(p.getSlot, other.getSlot)
      )
    } else if (
      other.getSlot < 6 && !other.isTransparent
    ) // other gets full face, we didn't shrink, flag rendermask
      {
        if (
          other.getSlot match {
