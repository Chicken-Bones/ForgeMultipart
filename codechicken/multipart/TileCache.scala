...
object TileCache
{
    ...
    def s(c:BlockCoord) = "("+c.x+", "+c.y+", "+c.z+")"
    ...
    def findTile(world:World, c:BlockCoord) = BlockMultipart.getTile(world, c.x, c.y, c.z) match {
        case null => apply(c) match {
            case Some(FlaggedTile(t, rem)) =>
                if(!rem)
                    System.err.println("Client multipart @"+s(c)+" vanished from world but was recovered. If possible causes can be identified, please report to the github issue tracker.")
                t
            case None =>
                throw new RuntimeException("DC: Client multipart @"+s(c)+" not found")
            case _ =>
                null
        }
        case t => t
